...
int main(void)
{
  ...
  RC522_Init(&hrc522);
  uint8_t rc522_version = 0U;
  if (RC522_CheckComm(&hrc522, &rc522_version)) {
    char buf[64];
    snprintf(buf, sizeof(buf), "[BOOT] RC522 communication OK. Version=0x%02X\r\n", rc522_version);
    LogText(buf);
  } else {
    char buf[64];
    snprintf(buf, sizeof(buf), "[BOOT] RC522 communication FAILED. Version=0x%02X\r\n", rc522_version);
    LogText(buf);
  }
